...
public class SampleTypeLimitsTest extends BaseWebDriverTest
{
    ...
    private void doSetup() throws Exception
    {
        ...
        TestDataGenerator dgen = new SampleTypeDefinition(SAMPLE_TYPE_NAME)
                .addField(new FieldDefinition("label", ColumnType.String))
    ...
    public void testStringLookupFields() throws IOException, CommandException
    {
        ...
        listDef.addField(new FieldDefinition("lookUpField",
                new LookupInfo(null, "exp.materials", "10000Samples")
    ...
    public void testDeriveSamplesLookupFields() throws IOException, CommandException
    {
        ...
        definition.addField(new FieldDefinition("label", ColumnType.String));
    ...
    {
        final String sampleTypeName = "bigLineage";

        goToProjectHome();
        // create a sampleset with the following explicit domain columns
        TestDataGenerator dgen = new SampleTypeDefinition(sampleTypeName)
                .setFields(List.of(
                        new FieldDefinition("data", ColumnType.Integer),
                        new FieldDefinition("testIndex", ColumnType.Integer)))
                .create(createDefaultConnection(), getCurrentContainerPath());

        Map<String, Object> indexRow = Map.of("name", "seed", "data", dgen.randomInt(3, 2000), "testIndex", 0); // create the first seed in the lineage
        SaveRowsResponse seedInsert = dgen.insertRows(createDefaultConnection(), List.of(indexRow));
        SelectRowsResponse seedSelect = dgen.getQueryHelper(createDefaultConnection())
                .selectRows(List.of("lsid", "name", "parent", "data", "testIndex"));
        ...
        for (int i = 0; i < intendedGenerationDepth; i++)
        {
            ...
            Map<String, Object> row = Map.of("name", name, "data", dgen.randomInt(3, 1395), "testIndex", testIndex , "MaterialInputs/bigLineage", previousName);
            dgen.addCustomRow(row);
            previousName = name;
            testIndex++;
        }
        dgen.insertRows(createDefaultConnection(), dgen.getRows());
        dgen.getQueryHelper(createDefaultConnection()).selectRows(List.of("name", "data", "testIndex"));
